...
# เปลี่ยนปลายทางให้เป็น bucket ใน MinIO เช่น s3a://<bucket-name>/output_data
STORAGE_PATH = os.getenv("STORAGE_PATH", 's3a://streamify-data/output_data')

# initialize a spark session with MinIO
spark = get_spark_minio('Eventsim Stream')
...
listen_events_writer = create_file_write_stream(listen_events,
                                                f"{STORAGE_PATH}/{LISTEN_EVENTS_TOPIC}",
                                                f"{STORAGE_PATH}/checkpoint/{LISTEN_EVENTS_TOPIC}"
                                                )

page_view_events_writer = create_file_write_stream(page_view_events,
                                                   f"{STORAGE_PATH}/{PAGE_VIEW_EVENTS_TOPIC}",
                                                   f"{STORAGE_PATH}/checkpoint/{PAGE_VIEW_EVENTS_TOPIC}"
                                                   )

auth_events_writer = create_file_write_stream(auth_events,
                                              f"{STORAGE_PATH}/{AUTH_EVENTS_TOPIC}",
                                              f"{STORAGE_PATH}/checkpoint/{AUTH_EVENTS_TOPIC}"
                                              )
